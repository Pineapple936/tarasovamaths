...
export default function ScrollWrapper({
  ...
  return (
    <div className={style.wrapper}>
      ...
      >
        <div
          className={style.lenta}
          style={{
            transform: `translateX(calc(-${activeIndex} * (var(--widthBlock) + var(--gap))))`,
          }}
        >
          {children}
        </div>
      ...
      <div className={style.buttonsWrapper}>
        <button onClick={() => setActiveIndex((prev) => (prev - 1 + countItems) % countItems)}>
          <i className="bx bx-chevron-left" />
        </button>
        <div className={style.lineButton}>{buttons}</div>
        <button
          className={style.buttonright}
          onClick={() => setActiveIndex((prev) => (prev + 1) % countItems)}
        >
